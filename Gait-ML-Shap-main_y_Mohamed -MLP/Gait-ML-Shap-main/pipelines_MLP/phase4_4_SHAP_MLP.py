...
def save_importance_plot(summary_df, output_dir, top_n=15):
    ...
    plt.ylabel("Feature")
    plt.title("Top MLP Feature Importance")
    plt.grid(axis="x", alpha=0.25)
    plt.tight_layout()

    plot_file = os.path.join(output_dir, "mlp_permutation_importance_top_features.png")
...
def save_confusion_matrix_plot(cm, label_classes, output_dir):
    ...
    plt.title("MLP Evaluation Confusion Matrix")
    ...
    plot_file = os.path.join(output_dir, "mlp_evaluation_confusion_matrix.png")
    plt.savefig(plot_file, dpi=300)
    plt.close()
...
def main():
    logger.info(">>>>>>>>>> Starting MLP Phase 4.4: MLP Interpretation <<<<<<<<<<")
    ...
    n_repeats = int(mlp_config.get("permutation_repeats", 30))
    ...
    output_file = os.path.join(
        PROJECT_ROOT,
        config["data"]["output_file_mlp"],
    )

    output_dir = os.path.join(
        PROJECT_ROOT,
        config["data"]["output_dir_mlp"],
    )
    ...
    if top3_df.empty:
        raise ValueError("Top3 MLP file is empty.")
    ...
    logger.info(f"Loaded best MLP config: {model_config}")
    ...
    logger.info(f"Saved MLP interpretation Excel to: {output_file}")
    ...
    logger.info("Top MLP important features:")
    ...
    logger.info(">>>>>>>>>> Completed MLP Phase 4.4: MLP Interpretation <<<<<<<<<<")
...
if __name__ == "__main__":
    try:
        ...
        logger.error(f"An error occurred in MLP Phase 4.4: {e}")
        raise e
