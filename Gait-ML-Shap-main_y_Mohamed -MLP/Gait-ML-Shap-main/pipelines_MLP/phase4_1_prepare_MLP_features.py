...
def create_mlp_feature_sets(
    ...
    """
    Create MLP feature subsets using ANOVA F-score ranking.
    ...
    if results_df.empty:
        raise ValueError("No MLP feature sets were created.")
    ...
    with pd.ExcelWriter(output_file, engine="openpyxl") as writer:
        results_df.to_excel(
            writer,
            sheet_name="MLP_Feature_Sets",
            index=False,
        )

    logger.info(f"Saved MLP Phase 4.1 results to: {output_file}")
...
def main():
    logger.info(">>>>>>>>>> Starting MLP Phase 4.1: Preparing MLP Feature Sets <<<<<<<<<<")

    config_path = os.path.join(PROJECT_ROOT, "configs", "config.yaml")
    config = read_yaml(config_path)

    group_column = config.get("SFS", {}).get("labels_column", "Group")
    participant_column = config.get("MLP", {}).get("participant_column", "Participant")
    ...
    logger.info(">>>>>>>>>> Completed MLP Phase 4.1: Preparing MLP Feature Sets <<<<<<<<<<")
...
if __name__ == "__main__":
    try:
        ...
        logger.error(f"An error occurred in MLP Phase 4.1: {e}")
        raise e
