...
def build_sort_columns(df):
    """
    Choose sorting columns that exist in the MLP result file.
    """
    preferred_sort = [
...
def add_readable_model_name(df):
    ...
    if "Name_Model" not in df.columns:
        ...
        for _, row in df.iterrows():
            ...
            hidden_layers = config.get("hidden_layers", "NA")
            dropout = config.get("dropout", "NA")
            lr = config.get("learning_rate", "NA")

            names.append(
                f"MLP_Candidate_{candidate_id}_"
                f"{num_features}F_"
                f"HL{hidden_layers}_"
                f"D{dropout}_"
                f"LR{lr}"
            )
...
def select_top3_by_feature_count(df, feature_counts=None, top_n=3):
    ...
    for count in feature_counts:
        ...
        if subset.empty:
            logger.warning(f"No MLP results found for {count} features.")
            continue
...
def save_excel_outputs(
    ...
    with pd.ExcelWriter(scores_file, engine="openpyxl") as writer:
        ranked_df.to_excel(writer, sheet_name="Ranked_All_MLP", index=False)
    ...
    logger.info(f"Saved ranked MLP scores to: {scores_file}")
    logger.info(f"Saved Top 3 MLP scores to: {top3_file}")
...
def main():
    logger.info(">>>>>>>>>> Starting MLP Phase 4.3: Selecting Best MLP Scores <<<<<<<<<<")
    ...
    if not os.path.exists(input_file):
        raise FileNotFoundError(f"MLP Phase 4.2 result file not found: {input_file}")

    logger.info(f"Reading MLP Phase 4.2 results from: {input_file}")

    df = pd.read_excel(input_file)

    if df.empty:
        raise ValueError("MLP Phase 4.2 results file is empty.")
    ...
    if missing_columns:
        raise ValueError(f"Missing required columns in MLP results: {sorted(missing_columns)}")
    ...
    logger.info(f"Sorting MLP models by: {sort_columns}")
    ...
    requested_feature_counts = mlp_config.get("top3_feature_counts", [5, 6, 10])
    ...
    logger.info("Top 3 MLP overall models:")
    ...
    logger.info(">>>>>>>>>> Completed MLP Phase 4.3: Selecting Best MLP Scores <<<<<<<<<<")
...
if __name__ == "__main__":
    try:
        ...
        logger.error(f"An error occurred in MLP Phase 4.3: {e}")
        raise e
