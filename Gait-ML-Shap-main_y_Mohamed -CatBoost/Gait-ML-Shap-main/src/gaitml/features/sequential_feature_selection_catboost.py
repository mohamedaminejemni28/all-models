"""
sequential_feature_selection_catboost.py

CatBoost version of Sequential Feature Selection.

This file is used by:
    pipelines_CATBOOST/phase4_1_SFS_CATBOOST.py
"""
...
def get_numeric_feature_data(
    ...
    Remove Group and Participant columns to avoid leakage.
    """
...
def save_partial_results(results_per_sheet, output_excel_file):
    """
    Save partial results during SFS.
...
def sequential_feature_selection_catboost(
    input_excel_file,
    output_excel_file,
    group_column="Group",
    classes_dict=None,
    ...
    """
    Run CatBoost-based Sequential Feature Selection.

    The goal is to generate feature subsets for:
        5, 6, 7, ..., 19 features

    Output rows:
        one row = one CatBoost configuration + one selected feature subset
    """
    ...
    if (
        ...
        and len(parameter_grid) > max_candidates
    ):
    ...
    print(
        f"CatBoost SFS starting | sheets will use {len(parameter_grid)} parameter combinations | max_features={requested_max_features}",
        flush=True,
    )
    ...
    for sheet_name in excel_file.sheet_names:
        ...
        local_n_splits = min(int(kfold_n_splits), min_class_count)

        if local_n_splits < 2:
            logger.warning(
                f"Sheet {sheet_name} has too few samples per class for CV. Skipping."
            )
            continue

        if local_n_splits != int(kfold_n_splits):
            logger.warning(
                f"Reducing CV folds from {kfold_n_splits} to {local_n_splits} for sheet {sheet_name}."
            )

        print(
            f"Sheet {sheet_name}: {X_df.shape[0]} rows, {X_df.shape[1]} numeric features, CV folds={local_n_splits}",
            flush=True,
        )
        ...
        for combo_id, (
            ...
            print(
                f"[{combo_id}/{len(parameter_grid)}] CatBoost SFS | sheet={sheet_name} | iterations={iterations}, depth={depth}, lr={learning_rate}",
                flush=True,
            )

            logger.info(
                f"[{combo_id}/{len(parameter_grid)}] Running CatBoost SFS | "
                f"iterations={iterations}, "
                f"depth={depth}, "
                f"learning_rate={learning_rate}, "
                f"l2_leaf_reg={l2_leaf_reg}, "
                f"bagging_temperature={bagging_temperature}, "
            ...
            logger.info(f"SFS fit completed for combo {combo_id}/{len(parameter_grid)} in {elapsed:.1f}s")
            print(
                f"[{combo_id}/{len(parameter_grid)}] SFS fit done in {elapsed:.1f}s; scoring requested feature counts...",
                flush=True,
            )
            ...
            for feature_count in local_feature_counts:
                if feature_count not in sfs_results:
                    logger.warning(
                        f"SFS result for {feature_count} features not found."
                    )
                    continue

                selected_indices = list(
                    sfs_results[feature_count]["feature_idx"]
                )

                selected_indices = [int(index) for index in selected_indices]

                selected_feature_names = [
                    feature_names_all[index]
            ...
            if save_partial:
                ...
                logger.info(
                    f"Partial CatBoost SFS results saved after combo "
                    f"{combo_id}/{len(parameter_grid)}"
                )
                print(
                    f"Saved partial SFS results: {output_excel_file}",
                    flush=True,
                )

    logger.info(f"Saving final CatBoost SFS results to: {output_excel_file}")
    ...
    logger.info("CatBoost SFS completed successfully.")
