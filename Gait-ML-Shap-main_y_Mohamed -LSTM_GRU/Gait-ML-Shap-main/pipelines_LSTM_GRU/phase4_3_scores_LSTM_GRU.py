...
def build_sort_columns(df):
    """
    Choose sorting columns that exist in the LSTM_GRU result file.
    """
    preferred_sort = [
...
def add_readable_model_name(df):
    ...
    if "Name_Model" not in df.columns:
        ...
        for _, row in df.iterrows():
            ...
            cell_type = config.get("cell_type", "NA")
            hidden_size = config.get("hidden_size", "NA")
            num_layers = config.get("num_layers", "NA")
            dropout = config.get("dropout", "NA")
            lr = config.get("learning_rate", "NA")

            names.append(
                f"LSTM_GRU_Candidate_{candidate_id}_"
                f"{num_features}F_"
                f"Cell{cell_type}_"
                f"Hidden{hidden_size}_"
...
def select_top3_by_feature_count(df, feature_counts=None, top_n=3):
    ...
    for count in feature_counts:
        ...
        if subset.empty:
            logger.warning(f"No LSTM_GRU results found for {count} features.")
            continue
...
def save_excel_outputs(
    ...
    with pd.ExcelWriter(scores_file, engine="openpyxl") as writer:
        ranked_df.to_excel(writer, sheet_name="Ranked_All_LSTM_GRU", index=False)
    ...
    logger.info(f"Saved ranked LSTM_GRU scores to: {scores_file}")
    logger.info(f"Saved Top 3 LSTM_GRU scores to: {top3_file}")
...
def main():
    logger.info(">>>>>>>>>> Starting LSTM_GRU Phase 4.3: Selecting Best LSTM_GRU Scores <<<<<<<<<<")
    ...
    if not os.path.exists(input_file):
        raise FileNotFoundError(f"LSTM_GRU Phase 4.2 result file not found: {input_file}")

    logger.info(f"Reading LSTM_GRU Phase 4.2 results from: {input_file}")

    df = pd.read_excel(input_file)

    if df.empty:
        raise ValueError("LSTM_GRU Phase 4.2 results file is empty.")
    ...
    if missing_columns:
        raise ValueError(f"Missing required columns in LSTM_GRU results: {sorted(missing_columns)}")
    ...
    logger.info(f"Sorting LSTM_GRU models by: {sort_columns}")
    ...
    requested_feature_counts = LSTM_GRU_config.get("top3_feature_counts", [5, 6, 10])
    ...
    logger.info("Top 3 LSTM_GRU overall models:")
    ...
    logger.info(">>>>>>>>>> Completed LSTM_GRU Phase 4.3: Selecting Best LSTM_GRU Scores <<<<<<<<<<")
...
if __name__ == "__main__":
    try:
        ...
        logger.error(f"An error occurred in LSTM_GRU Phase 4.3: {e}")
        raise e
