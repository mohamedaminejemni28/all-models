...
def save_importance_plot(summary_df, output_dir, top_n=15):
    ...
    plt.ylabel("Feature")
    plt.title("Top LSTM_GRU Feature Importance")
    plt.grid(axis="x", alpha=0.25)
    plt.tight_layout()

    plot_file = os.path.join(output_dir, "LSTM_GRU_permutation_importance_top_features.png")
...
def save_confusion_matrix_plot(cm, label_classes, output_dir):
    ...
    plt.title("LSTM_GRU Evaluation Confusion Matrix")
    ...
    plot_file = os.path.join(output_dir, "LSTM_GRU_evaluation_confusion_matrix.png")
    plt.savefig(plot_file, dpi=300)
    plt.close()
...
def main():
    logger.info(">>>>>>>>>> Starting LSTM_GRU Phase 4.4: LSTM_GRU Interpretation <<<<<<<<<<")
    ...
    n_repeats = int(LSTM_GRU_config.get("permutation_repeats", 30))
    ...
    checkpoint_file = os.path.join(
        PROJECT_ROOT,
        "results",
        "LSTM_GRU_checkpoints",
        "best_LSTM_GRU_model.pt",
    )

    output_file = os.path.join(
        PROJECT_ROOT,
        config["data"]["output_file_LSTM_GRU"],
    )

    output_dir = os.path.join(
        PROJECT_ROOT,
        config["data"]["output_dir_LSTM_GRU"],
    )
    ...
    if top3_df.empty:
        raise ValueError("Top3 LSTM_GRU file is empty.")
    ...
    logger.info(f"Loaded best LSTM_GRU config: {model_config}")
    ...
    logger.info(f"Saved LSTM_GRU interpretation Excel to: {output_file}")
    ...
    logger.info("Top LSTM_GRU important features:")
    ...
    logger.info(">>>>>>>>>> Completed LSTM_GRU Phase 4.4: LSTM_GRU Interpretation <<<<<<<<<<")
...
if __name__ == "__main__":
    try:
        ...
        logger.error(f"An error occurred in LSTM_GRU Phase 4.4: {e}")
        raise e
