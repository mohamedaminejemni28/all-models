...
def create_LSTM_GRU_feature_sets(
    train_file,
    output_file,
    ...
    """
    Create LSTM_GRU feature subsets using ANOVA F-score ranking.
    ...
    if results_df.empty:
        raise ValueError("No LSTM_GRU feature sets were created.")
    ...
    with pd.ExcelWriter(output_file, engine="openpyxl") as writer:
        results_df.to_excel(
            writer,
            sheet_name="LSTM_GRU_Feature_Sets",
            index=False,
        )

    logger.info(f"Saved LSTM_GRU Phase 4.1 results to: {output_file}")
...
def main():
    logger.info(">>>>>>>>>> Starting LSTM_GRU Phase 4.1: Preparing LSTM_GRU Feature Sets <<<<<<<<<<")

    config_path = os.path.join(PROJECT_ROOT, "configs", "config.yaml")
    config = read_yaml(config_path)

    group_column = config.get("SFS", {}).get("labels_column", "Group")
    participant_column = config.get("LSTM_GRU", {}).get("participant_column", "Participant")
    ...
    logger.info(">>>>>>>>>> Completed LSTM_GRU Phase 4.1: Preparing LSTM_GRU Feature Sets <<<<<<<<<<")
...
if __name__ == "__main__":
    try:
        ...
        logger.error(f"An error occurred in LSTM_GRU Phase 4.1: {e}")
        raise e
