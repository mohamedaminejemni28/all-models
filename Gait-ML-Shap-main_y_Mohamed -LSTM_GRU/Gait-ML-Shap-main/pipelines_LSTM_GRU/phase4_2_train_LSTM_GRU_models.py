...
class GaitRecurrentClassifier(nn.Module):
    def __init__(
        self,
        num_features,
        num_classes=2,
        ...
        head_size = hidden_size * (2 if bidirectional else 1)
    ...
    def forward(self, x):
        sequence = self.input_projection(x.unsqueeze(-1))
...
def build_model(model_config, num_features, num_classes=2):
    return GaitRecurrentClassifier(
...
def train_with_early_stopping(
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        with torch.no_grad():
            ...
            valid_loss = criterion(valid_logits, y_valid_tensor).item()
...
def generate_LSTM_GRU_candidates(feature_sets_df, LSTM_GRU_config, seed=42):
    """
    Generate LSTM_GRU candidate models.
    Each row = one LSTM_GRU experiment.
    """
    random.seed(seed)
    np.random.seed(seed)

    feature_counts = LSTM_GRU_config.get("feature_counts", None)
    ...
    if feature_sets_df.empty:
        raise ValueError("No feature sets available for LSTM_GRU training.")

    cell_type_list = LSTM_GRU_config.get("cell_type", ["GRU", "LSTM"])
    hidden_size_list = LSTM_GRU_config.get("hidden_size", [32, 64, 128])
    num_layers_list = LSTM_GRU_config.get("num_layers", [1, 2])
    dropout_list = LSTM_GRU_config.get("dropout", [0.1, 0.2, 0.3])
    bidirectional_list = LSTM_GRU_config.get("bidirectional", [False])
    learning_rate_list = LSTM_GRU_config.get("learning_rate", [0.001, 0.0005, 0.0001])
    batch_size_list = LSTM_GRU_config.get("batch_size", [4, 8, 16])
    weight_decay_list = LSTM_GRU_config.get("weight_decay", [0.0, 0.0001, 0.001])

    max_candidates = LSTM_GRU_config.get("max_candidates", 1000)
    ...
    for _, feature_row in feature_sets_df.iterrows():
        ...
        for cell_type, hidden_size, num_layers, dropout, bidirectional, lr, batch_size, weight_decay in itertools.product(
            cell_type_list,
            hidden_size_list,
            num_layers_list,
            dropout_list,
...
def main():
    logger.info(">>>>>>>>>> Starting LSTM_GRU Phase 4.2: Training LSTM_GRU Models <<<<<<<<<<")
    ...
    LSTM_GRU_config = config.get("LSTM_GRU", {})

    seed = int(LSTM_GRU_config.get("random_state", 42))
    set_seed(seed)

    group_column = config.get("SFS", {}).get("labels_column", "Group")
    participant_column = LSTM_GRU_config.get("participant_column", "Participant")

    strict_subject_split = bool(LSTM_GRU_config.get("strict_subject_split", True))
    n_splits = int(LSTM_GRU_config.get("n_splits", 5))
    max_epochs = int(LSTM_GRU_config.get("max_epochs", 150))
    patience = int(LSTM_GRU_config.get("patience", 20))
    ...
    output_file = os.path.join(
        PROJECT_ROOT,
        config["data"]["combination_results_file_LSTM_GRU"],
    )

    top3_output_file = os.path.join(
        PROJECT_ROOT,
        config["data"]["scores_file_top3_LSTM_GRU"],
    )

    final_scores_file = os.path.join(
        PROJECT_ROOT,
        config["data"]["scores_file_LSTM_GRU"],
    )

    checkpoint_dir = os.path.join(PROJECT_ROOT, "results", "LSTM_GRU_checkpoints")
    os.makedirs(checkpoint_dir, exist_ok=True)

    checkpoint_file = os.path.join(checkpoint_dir, "best_LSTM_GRU_model.pt")
    ...
    candidates_df = generate_LSTM_GRU_candidates(
        feature_sets_df=feature_sets_df,
        LSTM_GRU_config=LSTM_GRU_config,
        seed=seed,
    )

    logger.info(f"Number of LSTM_GRU candidates: {len(candidates_df)}")
    ...
    logger.info(f"Saved all LSTM_GRU results to: {output_file}")
    logger.info(f"Saved Top 3 LSTM_GRU results to: {top3_output_file}")
    ...
    logger.info(f"Best LSTM_GRU config: {best_config}")
    ...
    logger.info(f"Saved final LSTM_GRU scores to: {final_scores_file}")
    logger.info(f"Saved LSTM_GRU checkpoint to: {checkpoint_file}")
    ...
    if test_cm is not None:
        ...
        axes[0].set_title("LSTM_GRU Final Training Loss")
        ...
        ax.set_title("LSTM_GRU Final Training Loss")
        ax.set_xlabel("Epoch")
        ax.set_ylabel("Loss")
        ax.grid(alpha=0.25)
    ...
    plot_file = os.path.join(PROJECT_ROOT, "results", "LSTM_GRU_final_training_plot.png")
    plt.savefig(plot_file)
    plt.close()

    logger.info(f"Saved LSTM_GRU final plot to: {plot_file}")

    logger.info(">>>>>>>>>> Completed LSTM_GRU Phase 4.2: Training LSTM_GRU Models <<<<<<<<<<")
...
if __name__ == "__main__":
    try:
        ...
        logger.error(f"An error occurred in LSTM_GRU Phase 4.2: {e}")
        raise e
